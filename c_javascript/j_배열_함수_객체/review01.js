//! todo 객체 리스트 만들기 ( 할 일 목록 관리 시스템 )
//, todo 객체 구성

// - id  : 각 할 일의 고유 식별자 주로 //*숫자나 문자열로 표현
// - content : 할 일의 대용을 //*문자열로 저장
// - completed : 완료 여부 //*boolean 값
...
//# 프로젝트 구조
// 1. 할 일 추가 할수 있어야한다. : 객체를 사용하여 할 일을 저장하고 배열에 객체를 추가
...
function addTodo (content) { //함수 호출 시 인자로 할 일의 내용을 전달받음 
  // 새로운 할 일 생성
  const newTodo = {
    id : todos.length + 1, // 인덱스 번호 + 1 ( 이전 배열의 길이 + 1)
    content: content,
    completed: false
  }
...
function toggleTodo (id) {
  todos = todos.map((todo) => {
    // 순회되는 할 일의 id와 매개변수로 전달받은 id가 일치한다면 
    if (todo.id === id) {
      ...
      // 속성키 : 새로운 속성값; >> 객체 내부에서 속성값 재할당

      return {...todo, completed: !todo.completed}; // todo는 현재 순회되는 객체 (할일)값
    }
